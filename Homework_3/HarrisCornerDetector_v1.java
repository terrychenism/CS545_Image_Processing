...
public class HarrisCornerDetector {
	...
	public double CalContrast(ImageProcessor ip){
		int w = ip.getWidth();
		int h = ip.getHeight();
		int sum = 0;
		...
		double aveValue= sum/(w*h*255);
		double  mean= 0; 
		for (int v = 0; v < h; v++) {
			for (int u = 0; u < w; u++) {
				int ii = ip.getPixel(u, v);
				mean = mean + (ii/255-aveValue)*(ii/255-aveValue);				
			}			
		}		
		return Math.sqrt(mean/(w*h));
...
//		FloatProcessor Iy = (FloatProcessor) ipOrig.convertToFloat();
	...
	private List<Corner> collectCorners(int border) {
		...
		double contrast  = CalContrast(ipOrig);
		double threshold = 0.005*contrast;
